...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  return {
    ...
    image_max_concurrent_requests: Number(config.image_max_concurrent_requests ?? 5),
    image_max_concurrent_per_user: Number(config.image_max_concurrent_per_user ?? 2),
    image_account_cooldown_seconds: Number(config.image_account_cooldown_seconds ?? 3),
  ...
  saveConfig: async () => {
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        show_image_model_selector: config.show_image_model_selector !== false,
        upstream_image_channels_enabled: config.upstream_image_channels_enabled !== false,
        image_max_concurrent_requests: Math.max(0, Number(config.image_max_concurrent_requests) || 0),
        image_max_concurrent_per_user: Math.max(0, Number(config.image_max_concurrent_per_user) || 0),
        image_account_cooldown_seconds: Math.max(0, Number(config.image_account_cooldown_seconds) || 0),
